...
#define SWAP(a, b)  do { decltype(a) t; t = a; a = b; b = t; } while(0)
...
{
   _declspec(align(16)) int c[8];
	
#ifdef USE_SSE
	vec4_t ta, tb;

	ta = _mm_mul_ps(a, V4GRID);
	ta = _mm_add_ps(ta,V4HALF);
	
	tb = _mm_mul_ps(b, V4GRID);
	tb = _mm_add_ps(tb, V4HALF);
...
static void dxtblock_init(dxtblock_t *dxtb, const unsigned char *block, int flags)
{
   ...
   // select diagonal
   center = _mm_add_ps(max, min);
   center = _mm_mul_ps(center,V4HALF);

   cov = vec4_zero();
   for(i = 0; i < 16; ++i)
   {
	   t = _mm_sub_ps(dxtb->points[i], center);
	   
	   vec4_t temp = _mm_mul_ps(t, vec4_splatz(t));
	   cov = _mm_add_ps(cov, temp);
   ...
   inset = _mm_sub_ps(max, min);
   inset = _mm_mul_ps(inset, vec4_set1(1.0f / 16.0f));

   inset = _mm_sub_ps(inset, vec4_set1((8.0f / 255.0f) / 16.0f));

   //inset = (max - min) * vec4_set1(1.0f / 16.0f) - vec4_set1((8.0f / 255.0f) / 16.0f);
   max = vec4_min(V4ONE, vec4_max(V4ZERO, _mm_sub_ps(max,inset)));
   min = vec4_min(V4ONE, vec4_max(V4ZERO, _mm_add_ps(min,inset)));

   // clamp to color space and save

   vec4_t temp = _mm_mul_ps(max, V4GRID);
   temp = _mm_add_ps(temp, V4HALF);

   dxtb->max = _mm_mul_ps(vec4_trunc(temp), V4GRIDRCP);

   temp = _mm_mul_ps(min, V4GRID);
   temp = _mm_add_ps(temp, V4HALF);

   dxtb->min = _mm_mul_ps(vec4_trunc(temp), V4GRIDRCP);
...
static void construct_palette3(dxtblock_t *dxtb)
{
   ...
   vec4_t left = _mm_mul_ps(dxtb->max, V4HALF);
   vec4_t right = _mm_mul_ps(dxtb->min,V4HALF);
   dxtb->palette[2] = _mm_add_ps(left, right);
...
static void construct_palette4(dxtblock_t *dxtb)
{
   ...
   vec4_t left = _mm_mul_ps(dxtb->max, V4TWOTHIRDS);
   vec4_t right = _mm_mul_ps(dxtb->min, V4ONETHIRD);

   dxtb->palette[2] = _mm_add_ps(left, right);

   left = _mm_mul_ps(dxtb->max, V4ONETHIRD);
   right = _mm_mul_ps(dxtb->min, V4TWOTHIRDS);

   dxtb->palette[3] = _mm_add_ps(left, right);
}
                                ...
                                vec4_t *max, vec4_t *min)
{
   ...
   for(i = 0; i < 16; ++i)
   {
	  ...
	  vec4_t mul = _mm_mul_ps(a, a);
	  alpha2_sum = _mm_add_ps(alpha2_sum, mul);
	  beta2_sum = _mm_add_ps(beta2_sum, _mm_mul_ps(b, b));
	  alphabeta_sum = _mm_add_ps(alphabeta_sum, _mm_mul_ps(a, b));

	  alphax_sum = _mm_add_ps(alphax_sum, _mm_mul_ps(dxtb->points[i], a));
	  betax_sum = _mm_add_ps(betax_sum, _mm_mul_ps(dxtb->points[i], b));
	  /*
	  alpha2_sum += a * a;
      beta2_sum += b * b;
      alphabeta_sum += a * b;
   ...
   factor = _mm_sub_ps(_mm_mul_ps(alpha2_sum, beta2_sum), _mm_mul_ps(alphabeta_sum, alphabeta_sum));
   //factor = alpha2_sum * beta2_sum - alphabeta_sum * alphabeta_sum;
   if(vec4_cmplt(factor, V4EPSILON)) return;
   factor = vec4_rcp(factor);

   a = _mm_mul_ps( _mm_sub_ps(_mm_mul_ps(alphax_sum, beta2_sum), _mm_mul_ps(betax_sum, alphabeta_sum) ), factor);

   b = _mm_mul_ps(_mm_sub_ps(_mm_mul_ps(betax_sum, alpha2_sum), _mm_mul_ps(alphax_sum, alphabeta_sum)), factor);
   ...
   vec4_t temp = _mm_add_ps(_mm_mul_ps(V4GRID, a), V4HALF);

   a = _mm_mul_ps(temp, V4GRIDRCP);

   temp = _mm_add_ps(_mm_mul_ps(V4GRID, b), V4HALF);

   b = _mm_mul_ps(temp, V4GRIDRCP);
...
static unsigned int match_colors3(dxtblock_t *dxtb)
{
   ...
   for(i = 0; i < 16; ++i)
   {
	  ...
	  t0 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[0]), dxtb->metric);
	  t1 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[1]), dxtb->metric);
	  t2 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[2]), dxtb->metric);
	  ...
	  */
	  ...
	  d = _mm_add_ps(_mm_mul_ps(t0, t0), _mm_mul_ps(t1, t1));
	  d = _mm_add_ps(d, _mm_mul_ps(t2, t2));
...
static unsigned int match_colors4(dxtblock_t *dxtb)
{
   ...
   for(i = 0; i < 16; ++i)
   {
	   ...
	   t1 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[1]), dxtb->metric);
	   t2 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[2]), dxtb->metric);
	   t3 = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[3]), dxtb->metric);
	   /*
      t0 = (dxtb->points[i] - dxtb->palette[0]) * dxtb->metric;
      t1 = (dxtb->points[i] - dxtb->palette[1]) * dxtb->metric;
      t2 = (dxtb->points[i] - dxtb->palette[2]) * dxtb->metric;
...
static float compute_error3(dxtblock_t *dxtb, unsigned int indices)
{
   ...
   for(i = 0; i < 16; ++i)
   {
	  ...
	  t = _mm_mul_ps(_mm_sub_ps(dxtb->points[i], dxtb->palette[idx]), dxtb->metric);
...
static float compute_error4(dxtblock_t *dxtb, unsigned int indices)
{
   ...
   for(i = 0; i < 4; ++i)
   {
	  ...
	  a0 = _mm_mul_ps(_mm_sub_ps(a0, b0), dxtb->metric);
	  a1 = _mm_mul_ps(_mm_sub_ps(a1, b1), dxtb->metric);
	  a2 = _mm_mul_ps(_mm_sub_ps(a2, b2), dxtb->metric);
	  a3 = _mm_mul_ps(_mm_sub_ps(a3, b3), dxtb->metric);
      ...
      //d = a0 * a0 + a1 * a1 + a2 * a2;
	  d = _mm_add_ps(_mm_mul_ps(a0, a0), _mm_mul_ps(a1, a1));
	  d = _mm_add_ps(d, _mm_mul_ps(a2, a2));
...
extern "C"
{
	DDSPLUGIN_API int dxt_compress(unsigned char *dst, unsigned char *src, int format,
...
extern "C"
{
	DDSPLUGIN_API int dxt_decompress(unsigned char *dst, unsigned char *src, int format,
